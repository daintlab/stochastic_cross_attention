...
class OracleSelectionMethod(SelectionMethod):
    ...
    def run_acc(self, run_records):
        ...
        s0 = run_records.filter(lambda g: g['args']['seed'] == 0)
        s1 = run_records.filter(lambda g: g['args']['seed'] == 1)
        s2 = run_records.filter(lambda g: g['args']['seed'] == 2)

        if run_records[0]['args']['dataset'] == 'DomainNet':            
            s0 = s0.filter(lambda g: g['step'] in [1000, 2000, 3000, 4000, 5000])
            s1 = s1.filter(lambda g: g['step'] in [1000, 2000, 3000, 4000, 5000])
            s2 = s2.filter(lambda g: g['step'] in [1000, 2000, 3000, 4000, 5000])

        chosen_record0 = s0.sorted(lambda r: r[test_out_acc_key])[-1] 
        chosen_record1 = s1.sorted(lambda r: r[test_out_acc_key])[-1] 
        chosen_record2 = s2.sorted(lambda r: r[test_out_acc_key])[-1] 

        return {'val_acc': [chosen_record0[test_out_acc_key], chosen_record1[test_out_acc_key], chosen_record2[test_out_acc_key]],
                'test_acc': [chosen_record0[test_in_acc_key], chosen_record1[test_in_acc_key], chosen_record2[test_in_acc_key]]}
...
class IIDAccuracySelectionMethod(SelectionMethod):
    ...
    def run_acc(self, run_records):
        ...
        if not len(test_records):
            return None
        s0 = test_records.filter(lambda g: g['args']['seed'] == 0)
        s1 = test_records.filter(lambda g: g['args']['seed'] == 1)
        s2 = test_records.filter(lambda g: g['args']['seed'] == 2)
        ...
        s0_acc = s0.map(self._step_acc).argmax('val_acc')
        s1_acc = s1.map(self._step_acc).argmax('val_acc')
        s2_acc = s2.map(self._step_acc).argmax('val_acc')

        total = {'val_acc': [s0_acc['val_acc'], s1_acc['val_acc'], s2_acc['val_acc']],
                'test_acc': [s0_acc['test_acc'], s1_acc['test_acc'], s2_acc['test_acc']]}
