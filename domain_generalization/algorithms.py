...
## import CA methods
from domainbed.vit_cross_attention import vit_b_16 as vit_cross_att
from domainbed.vit_return_qkv import vit_b_16 as vit_ret_qkv

import clip
from domainbed.clip_cross_attention import build_model as clip_student
from domainbed.clip_return_qkv import build_model as clip_teacher
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.optimizer = torch.optim.AdamW(
            [
                ...
                {'params': self.classifier.parameters(), 'lr': self.hparams["lr"], 'weight_decay': self.hparams['weight_decay']}
            ]
        )
...
class StochCA(Algorithm):
    '''
    Stochastically apply CA or SA in each encoder layer
    '''
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(StochCA, self).__init__(input_shape, num_classes, num_domains, hparams)
        
        ca_prob = self.hparams['ca_prob']
        self.featurizer = vit_cross_att(pretrained=True, ca_prob=ca_prob)
        del self.featurizer.heads
        self.featurizer.heads = nn.Identity()
        self.classifier = networks.Classifier(
            768,
            num_classes,
            self.hparams['nonlinear_classifier'])
        
        self.pretrained_model = vit_ret_qkv(pretrained=True)
        del self.pretrained_model.heads
        ...
        modules = nn.ModuleList([self.featurizer, self.classifier])
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        with torch.no_grad():
            ...
            _, query, key, val = self.pretrained_model(all_x)
        
        # Forward with teachers k,v
        feat = self.featurizer(all_x, key=key, val=val)
...
class StochCA_CLIP(Algorithm):
    '''
    Stochastically apply CA or SA in each encoder layer
    '''
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(StochCA_CLIP, self).__init__(input_shape, num_classes, num_domains, hparams)
        ...
        featurizer_st = clip_student(ckpt, ca_prob)
        featurizer_st.float()
        self.featurizer = featurizer_st.visual
        self.classifier = nn.Linear(self.featurizer.output_dim, num_classes)
        
        featurizer_te = clip_teacher(ckpt)
        featurizer_te.float()
        self.pretrained_model = featurizer_te.visual
        for p in self.pretrained_model.parameters():
            p.requires_grad_(False)
        del featurizer_st
        del featurizer_te
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        with torch.no_grad():
            self.pretrained_model.eval()
            _, query, key, val,_ = self.pretrained_model(all_x)
        
        # Forward with teachers k,v
        feat = self.featurizer(all_x, key=key, val=val)
        feat = self.dropout(feat[0])
        loss = F.cross_entropy(self.classifier(feat), all_y)
