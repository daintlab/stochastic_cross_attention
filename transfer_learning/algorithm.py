...
# for stochCA
from vit_cross_attention import vit_b_16 as student_vit
...
class CoTuning_StochCA(nn.Module):
    def __init__(self, args):
        super(CoTuning_StochCA, self).__init__()
        self.args = args
        ca_prob = args.ca_prob
        ...
        self.optimizer = torch.optim.AdamW(
            [
                {'params' : self.student.parameters(), 'lr' : args.lr},
                {'params' : self.classifier_s.parameters(), 'lr' : args.lr},
                {'params' : self.classifier_t.parameters(), 'lr' : args.lr},
            ],
            weight_decay=args.wd
        )
...
class BSS_StochCA(nn.Module):
    def __init__(self, args):
        super(BSS_StochCA, self).__init__()
        self.args = args
        ca_prob = args.ca_prob
